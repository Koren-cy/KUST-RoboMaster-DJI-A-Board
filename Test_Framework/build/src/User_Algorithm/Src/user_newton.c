...
void Newton_Init(NEWTON_SOLVER* solver, const uint8_t n) {
    fprintf(stderr, "[NI] solver=%p n=%d\n", (void*)solver, n);
    solver->n = n;
    solver->eps = NEWTON_EPSILON;
    solver->max_iter = NEWTON_MAX_ITERS;
    fprintf(stderr, "[NI]   -> n=%d max_iter=%d eps=%e converged=%d\n",
            solver->n, solver->max_iter, (double)solver->eps, solver->converged);
}
...
uint8_t Newton_Solve(NEWTON_SOLVER* solver, const float* param, const float* x0) {
    fprintf(stderr, "[NS] solver=%p n=%d max_iter=%d\n",
            (void*)solver, solver->n, solver->max_iter);
    ...
    solver->iterations = 0;
    solver->converged = 0;
    fprintf(stderr, "[NS]   reset converged=0\n");

    for (uint16_t iter = 0; iter < solver->max_iter; iter++) {
        Newton_Evaluate(solver);
        fprintf(stderr, "[NS] iter %d: conv_before_check=%d res=%e\n",
                iter, solver->converged, (double)solver->residual);
        ...
        if (singular) {
            fprintf(stderr, "[NS] iter %d: singular=1, breaking\n", iter);
            break;
        }
    ...
    fprintf(stderr, "[NS] loop done: converged=%d iterations=%d\n",
